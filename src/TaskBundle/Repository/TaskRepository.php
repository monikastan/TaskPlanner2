...
class TaskRepository extends EntityRepository
{
    ...
    public function findTasks($id, $name, $desc, $dateFrom, $dateTo, $deadlineFrom, $deadlineTo, $priority, $category, $status){
        ...
        if( $name != null){ $q = $q." AND t.name LIKE :name"; }
        if( $desc != null){ $q = $q." AND t.description LIKE :desc";}
        if( $dateFrom != null){ $q = $q." AND t.date >= :dateFrom";}
        if( $dateTo != null){ $q = $q." AND t.date <= :dateTo";}
        if( $deadlineFrom != null){ $q = $q." AND t.deadline >= :deadlineFrom";}
        if( $deadlineTo != null){ $q = $q." AND t.deadline <= :deadlineTo";}
        if( $priority != null){ $q = $q." AND t.priority = :priority";}
        if( $category != null){ $q = $q." AND t.category = :category";}
        if( $status != null){ $q = $q." AND t.status = :status";}
        
        $query = $em->createQuery($q);
        
        $query->setParameter('id', $id);
        if( $name != null){ $query->setParameter('name', "%".  strtoupper(trim($name))."%"); }
        if( $desc != null){ $query->setParameter('desc', "%".strtoupper(trim($desc))."%");}
        if( $dateFrom != null){ $query->setParameter('dateFrom', $dateFrom);}
        if( $dateTo != null){ $query->setParameter('dateTo', $dateTo);}
        if( $deadlineFrom != null){ $query->setParameter('deadlineFrom', $deadlineFrom);}
        if( $deadlineTo != null){ $query->setParameter('deadlineTo', $deadlineTo);}
